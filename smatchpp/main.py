...
if __name__ == "__main__":
    ...
    """
    def process_pair(string_g1, string_g2):
        g1 = graph_reader.string2graph(string_g1)
        g2 = graph_reader.string2graph(string_g2)
        logger.debug("graph pair loaded,\n\nG1: {}\n\nG2: {}".format(g1, g2))
        g1 = graph_standardizer.standardize(g1)
        g2 = graph_standardizer.standardize(g2)
        logger.debug("graph pair standardized,\n\nG1: {}\n\nG2: {}".format(g1, g2))
        
        if args.score_dimension == "main":
            g1, g2, v1, v2 = graph_pair_prepairer.prepare_get_vars(g1, g2)
            logger.debug("graph pair fully prepared,\n\nG1: {}\n\nG2: {}\n\nVar G1: {}\n\nVar G2: {}".format(g1, g2, v1, v2))
            alignment, varindex, status = graph_aligner.align(g1, g2, v1, v2)
            logger.debug("alignment computed: {}; varindex: {}".format(alignment, varindex))
            match = graph_scorer.main_scores(g1, g2, alignment, varindex)

        elif args.score_dimension == "all-onealign":
            g1, g2, v1, v2 = graph_pair_prepairer.prepare_get_vars(g1, g2)
            logger.debug("graph pair fully prepared,\n\nG1: {}\n\nG2: {}\n\nVar G1: {}\n\nVar G2: {}".format(g1, g2, v1, v2))
            alignment, varindex, status = graph_aligner.align(g1, g2, v1, v2)
            logger.debug("alignment computed: {}; varindex: {}".format(alignment, varindex))
            match = graph_scorer.subtask_scores(g1, g2, alignment, varindex)

        elif args.score_dimension == "all-multialign":
            name_subgraph1 = subgraph_extractor.all_subgraphs_by_name(g1)
            name_subgraph2 = subgraph_extractor.all_subgraphs_by_name(g2)
            match = {}
            for name in name_subgraph1:
    ...
    if args.score_type == "pairwise":
        
        printer = eval_statistics.ResultPrinter(score_type=args.score_type, do_bootstrap=False, output_format=args.output_format)
        
        match_dict = {}
        for i, a in enumerate(amrs):
            
            match, status, alx = process_pair(a, amrs2[i])
        ...
        for i in range(len(amrs)):
            match_dict_tmp = {k:[match_dict[k][i]] for k in match_dict.keys()}
            printer.print_all(match_dict_tmp)
                

    if args.score_type == "macro":
    ...
    for status in STATUS:
        status_sum[0] += status[0]
        status_sum[1] += status[1]
